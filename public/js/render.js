  ...
  $('projects').innerHTML = state.apps.length ? state.apps.map((app) => {
    ...
    <div class="project-card">
      ...
      <div class="project-grid">
        ${[['Repository URL', app.repoUrl], ['Branch', app.branch], ['Subdirectory', app.subdir || '.'], ['Port', app.port], ['Local URL', app.url ? `<a href="${esc(app.url)}" target="_blank">${esc(app.url)}</a>` : '—'], ['SSH key', app.authMethod === 'ssh' ? esc(app.sshKeyLabel || 'System default / ssh-agent (not pinned)') : '—'], ['Stored .env', app.hasEnvFile ? 'present (written on deploy/start)' : 'none'], ['Remote URLs', remoteHtml], ['PM2 / PID', `${app.status} / ${app.pid || '—'}`], ['Last deploy', fmtDate(app.lastDeployAt)], ['Stored secret', app.hasSecret ? 'masked and stored locally' : 'none'], ['Last deploy result', app.lastDeploySummary || '—']].map(([k, v]) => `<div class="kv"><div class="k">${k}</div><div class="v">${v}</div></div>`).join('')}
      </div>
      <div class="actions">
